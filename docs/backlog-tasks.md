...
20. **Masked-dependency disclosure on backtracking** — `gcr[gtk]` / `xwayland[libei]` "All ebuilds masked" for a *dependency* atom. Same DFS-walk gap as #19. `TEST/findings/l0.md` cluster A/R. [A]
...
22. **`dep_zapdeps` finer choice bins** — `unsat_use_*` shipped 2026-09-11 (`||` group resolution + bug-515584 unmask gate, both languages); `other_*` bins and full `all_use_satisfied` remain. Real `depgraph.py` `_dep_zapdeps`. Affects `lib.rs` + `solver_bridge.rs` `||` closures. [A]
