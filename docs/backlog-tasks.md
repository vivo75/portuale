...
22. **`dep_zapdeps` finer choice bins** — `all_available`/`all_use_satisfied` split, `unsat_use_*` bins with the bug-515584 unmask gate and `all_installed_slots` shipped 2026-09-11 (both languages, one shared probe helper). Remaining: `other_*` bins + the `allow_masked` second pass, in-bin ordering (`|| ( foo:1 foo:2 )` upgrade promotion, `minimize_slots`), `conflict_downgrade`/`installed_downgrade`/`circular_atom`. Real `lib/portage/dep/dep_check.py::dep_zapdeps`. Code: `portage-repo` `disjunction_preference` + `portage-use-reduce` `resolve_disjunctions`; `solver_bridge.rs` is #34, not #22. Design brief for the remaining slices (in-bin ordering, `other_*`, downgrade/circular guards): `docs/022-agent-task-22-zapdeps.fable.md`. [A]
