...
24. **Slot-operator rebuild undo path** — add backtracking for a rebuild that shifts another sub-slot, re-bind the rebuilt consumer's own `:=` deps, and the `--changed-slot` interaction (one missing piece, not three). `slot_operator_rebuild_entries`; citations in `history/scope-backlog-2026-09-05.md`. S1-S3 shipped 2026-09-12 (plan `docs/024-slot-operator-plan.md`, oracle `docs/024-oracle.md`): S1 complete-mode gate under `--deep` (the a522084 `B-0` miss was the missing auto-enabled pass, not the undo path); S2 22 oracle pins over the whole `test_slot_operator_*` family; S3 the rebuild is now a **walked graph node** -- `slot_operator_rebuild_scan` feeds `BacktrackParams::slot_operator_replace_installed` through `collect_feedback`, the next pass seeds it like real's `@__auto_slot_operator_replace_installed__` `SetArg` and flips the outcome in-walk, so deps, `:=` re-bind and merge order are real (`[A-2, B-0, C-0]`, bug 522652); no synthesised entries left on the default solver. **Remaining: S4** (`_eliminate_rebuilds` undo, bug 614390 + `slotundo-unnecessary`), **S5** (`_slot_change_probe` + the `--changed-slot` rule-3 line), **S6** (L0 triage), **S7** (docs). [A]
