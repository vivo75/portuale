...
19. **DFS-partial merge-list truncation** — reproduce real's short merge list when a resolve is abandoned (autounmask + circular dep). Parked: branch `feat/circular-dep-backtracking`, notes in `history/dfs-graph-backtracker-exploration.md`, memory `circular-dep-backtracking-parked.md`. Needs a DFS resolution walk. [A]
...
22. **`dep_zapdeps` finer choice bins** — `all_available`/`all_use_satisfied` split, `unsat_use_*` bins with the bug-515584 unmask gate and `all_installed_slots` shipped 2026-09-11 (both languages, one shared probe helper); in-bin ordering (slice 2: upgrade-preference via `vercmp`/`cp_map`, `all_installed_slots`-over-any-slot, in-graph-over-installed-only) shipped 2026-09-11 via `use_reduce_flat_disjunctive`'s new `tie_break` parameter + `portage-repo::promote_tied_alternative`; the `other_*` bins + the `allow_masked` two-pass return (slice 4: `other_installed`/`other_installed_some`/`other_installed_any_slot`/`other`, `portage-repo::atom_matches_installed`) also shipped 2026-09-11 — when NO alternative anywhere is `all_available`, real (and now portuale) picks the best `other_*` bin instead of falling back to the literal `||`. Remaining: `minimize_slots` (needs `_overlap_dnf` DNF detection, not done), `conflict_downgrade`/`installed_downgrade`/`circular_atom`, `want_update` (needs the `--update`/`--newuse` "wanted" set threaded to this call depth — documented cut, pinned `false`), and the bug-600346 `cp_map` internal-consistency re-check (documented simplification: always keeps the higher version). Real `lib/portage/dep/dep_check.py::dep_zapdeps`. Code: `portage-repo` `disjunction_preference` + `promote_tied_alternative` + `portage-use-reduce` `resolve_disjunctions`; `solver_bridge.rs` is #34, not #22. Design brief for the remaining slice (downgrade/circular guards, `minimize_slots`): `docs/022-agent-task-22-zapdeps.fable.md`. [A]
23. **`_slot_conflict_backtrack` mask-target analysis** — richer choice of which version to mask when reconciling a slot conflict (real `depgraph.py`). Portuale's `'backtrack` loop. [A]
