...
24. **Slot-operator rebuild undo path** — add backtracking for a rebuild that shifts another sub-slot, re-bind the rebuilt consumer's own `:=` deps, and the `--changed-slot` interaction (one missing piece, not three). `slot_operator_rebuild_entries`; citations in `history/scope-backlog-2026-09-05.md`. S1-S5 shipped 2026-09-12 (plan `docs/024-slot-operator-plan.md`, oracle `docs/024-oracle.md`): S1 complete-mode gate under `--deep` (the a522084 `B-0` miss was the missing auto-enabled pass, not the undo path); S2 22 oracle pins over the whole `test_slot_operator_*` family; S3 the rebuild is now a **walked graph node** -- `slot_operator_rebuild_scan` feeds `BacktrackParams::slot_operator_replace_installed` through `collect_feedback`, the next pass seeds it like real's `@__auto_slot_operator_replace_installed__` `SetArg` and flips the outcome in-walk, so deps, `:=` re-bind and merge order are real (`[A-2, B-0, C-0]`, bug 522652); no synthesised entries left on the default solver; S4 `_eliminate_rebuilds` -- the nine ordered rules + the graph-aware `:=` binder (`bind_slot_operator_deps`, real `_eval_deps` over `_graph_trees`), demotion via `slot_operator_undone` latch, `slotundo-unnecessary` now MATCHes (`[U] souprov`, `abi_rebuilds: []`); S5 `_slot_change_probe` -- bug 456208's slot-move-without-revbump now schedules the installed child (`slotchange-1`/`regslotchange` MATCH, `[rR]` child + plain parent, no block), real's rule 3 landed behaviour-neutrally, and two S2 expectations were corrected live (the `--changed-slot` flag half has no `r`/edge in real; conflict-mass is v2 `#24b`'s update probe, not the probe). S4 finding: bug 614390's `complete` case is blocked by a **selection** gap (named bare `socc` resolves before meta's `=socc-1` through the already-installed fast path, which skips `resolved_slots`; real's `_add_pkg` slot-parent check catches it) -- #36-overlap, not the undo; stays strict-xfail. **Remaining: S6** (L0 triage), **S7** (docs). [A]
