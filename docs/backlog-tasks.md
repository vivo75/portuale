...
22. **`dep_zapdeps` finer choice bins** — port `unsat_use_*` / `other_*` bins and full `all_use_satisfied` for `||` branch selection (real `depgraph.py` `_dep_zapdeps`). Affects `lib.rs` + `solver_bridge.rs` `||` closures. [A]
