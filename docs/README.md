...
| [`batch-2026-09-21.md`](batch-2026-09-21.md) | **The current cross-tier batch plan** (30 items over tiers 1/2/3/6/8, grown from the original 16 as each phase filed its residues): the phase order and why, the per-slice runbook, the stop conditions, and pointers to the twelve per-cluster plan files (four still to write; Phase 0b lives in the index itself). An executing agent reads it plus one cluster file. |
