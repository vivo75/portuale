...
STATUS: DONE S0-S5 complete 2026-09-13(§11; TEST/findings/l2.md `#38 S0-S5`); S0 PORTAGE_TMPDIR finding⇒#37 follow-up LANDED; S4 user⇒IMPLEMENT instprep(¬#38b); S5 closeout⇒fresh L2 porttest 0unexpl(l2-20260913T203727Z 82known; xinst 67/67/0; ctrl 0/0)+L1 0hard(l1-20260913T204335Z 70mtime)+real-set 19binpkgs both PMs stop=#39(dep-rewrite 8rows)/`l2-binpkg-gpg-check`=NEW backlog #43+docs+step8 green; #37 LANDED (resolved FEATURES+PORTAGE_COMPRESS*+USE+SLOT in phase env)
...
§9 DOD: [x]S0 table+tool table+instprep call-order cited; expected trees frozen from O | [x]l2-gpkg-docompress closed(S1); l2-gpkg-dostrip-splitdebug* closed(S2)—each w/ before/after evidence | [x]RESTRICT=strip cell pinned by both-PM fixture(S3) | [x]instprep: implemented+fixtured(S4 run_instprep; repro l2-instprep-20260913T201638Z green) | [x]no transform reimplemented∈Rust(run_instprep=vendored __dyn_instprep invocation); no vendored bin/* edited | [x]L2 porttest 0-unexplained(l2-20260913T203727Z fresh; 82known incl 2 narrowed setuid-race rows hit); L1 unchanged(l1-20260913T204335Z 0 hard; both PMs 10 fixtures); full verification green(S5: fmt+clippy0w+cargo1025/0+pytest1567/2/8) | [x]docs updated(S5: what-this-proves §S4/S5; scope-backlog §K closed+GPG#43; backlog #38 DONE/#39 widened/#43 filed; README+ebuild doman claim corrected; docs/030 G0.5+S3); no dead allowlist entries(fresh xinst hit every yaml row: 50/10/2+4/1; setuid-race builder row hit by fresh archive diff)
...
§11 FINDINGS-LOG(append as S0-S5 run: cmd|expected|actual|root-cause|fix-ref|backlog-id)
  ...
  G6 2026-09-13 ∷ #37 follow-up LANDED ∷ PORTAGE_TMPDIR_DEFAULT=/var/tmp + portage_tmpdir_from_env/from_config(env→resolved Config::other_vars→real default) @pretend/ebuild/regen CLI boundaries; unit test portage_tmpdir_comes_from_the_resolved_scalar_then_the_make_globals_default
  S1 2026-09-13 ∷ l2-20260913T191450Z(L2_REBUILD=1,strict) rc0 ∷ l2-gpkg-docompress CLOSED: docs archives strict-clean modulo #39 metadata; O-tree(BIG.txt.bz2/small.txt/html/man/info) both; +Rust e2e install_qa_check_docompress_* +pytest docompress(CONTENTS .bz2); ¬product change
  S2 2026-09-13 ∷ same run ∷ l2-gpkg-dostrip-splitdebug* CLOSED: splitdebug path-set/modes/symlinks/sha256 = portage-sibling ∧ =2026-09-08 oracle build-ids 7c/… e8/…; setuid 14384B; diff only #39 metadata ⇒ deleted 8 broad KNOWN_FINDINGS + 5 broad yaml, kept 1 narrowed race row(``.build-id/e0/2277*``; real-divergence O pt-setuid vs real-L2 pt-sticky, ¬hit this run)
  S2-G3 2026-09-13 ∷ --shell brush smoke ∷ docs builds+ecompress fires(BIG.txt.bz2) BUT splitdebug ⇒ EMPTY image rc0(brush no-ops compiled fixture src_compile; work/ empty; declare readonly+``env: ''`` noise; bash same fixture OK)⇒brush-backend gap ¬estrip/ecompress, filed backlog-tasks #6/brush-pin workflow; brush stays, default bash
  S3 2026-09-13 ∷ same run ∷ porttest/restrict-strip(new EAPI8 RESTRICT=strip -g binary + md5-cache + atomlist + README) built both PMs ∷ strict-clean: pt-restrict 16576B byte-identical unstripped(build-id d69a589e…) ∧ ¬.debug objects(/usr/lib/debug; both leave empty dir) w/ FEATURES=splitdebug on ⇒ cell pinned; nostrip variant unfiled(bed ¬per-fixture FEATURES)
  S4 2026-09-13 ∷ user⇒IMPLEMENT ∷ repro TEST/run/l2-instprep-repro.sh(+layers/l2/instprep-repro.sh; FEATURES="-binpkg-dostrip -binpkg-docompress"; porttest/{docs,setuid}; cell src=emerge -1, cell bin=-B→unmerge→-1K; 1 container/PM) before l2-instprep-20260913T195828Z rc1(real BIG.txt.bz2+14384B stripped+.debug, archive 15424B; portuale ¬) ∷ real pos re-read 3.0.82.2: instprep=treewalk FIRST step(before INSTALL_MASK/collision/preinst), cmd misc_sh ∷ fix ebuild_phases::run_instprep(__dyn_instprep, EBUILD_PHASE=instprep, env=MergeOptions::build_env; bin: seed_saved_environment+binary_merge_env) @top merge_after_install(all src merges) + merge_binpkg after setup before INSTALL_MASK; !=0⇒"!!! instprep failed" rc1; bin w/o saved env⇒¬phase(hook degrade) ∷ after l2-instprep-20260913T201638Z rc0 ∷ tests Rust run_instprep_applies_only_the_complement_of_the_install_qa_gate + pytest test_emerge_instprep_compresses_at_merge_for_source_and_binary ∷ L2 l2-20260913T201941Z 80known/0unexpl(=S3), L1 l1-20260913T202308Z 0hard(=S3) ∷ FILED #42: emerge ¬pre-clean PORTAGE_BUILDDIR(real EbuildBuild._start_pre_clean)⇒dirty rebuild reuses instprepped image(-B archive stripped, -K estrip "debuglink section already exists"); scheduler scope; repro wipes builddir between cells
  S5 2026-09-13 ∷ closeout ∷ fresh porttest L2_REBUILD=1 l1-porttest.txt⇒l2-20260913T203727Z rc0 82known/0unexpl(2 extra known=narrowed setuid build-id race HIT: real e0/2277…→pt-setgid vs portuale pt-setuid ∧ oracle pt-setuid=real-vs-itself, ¬portuale-vs-real); xinst 67/67expl/0unexpl(=S2), ctrl 0/0, 10 fixture archive pairs(=+builder's portage gpkg⇒11/11) ∷ L1 l1-merge-from-binpkg.sh l1-porttest.txt⇒l1-20260913T204335Z rc0 0hard/70mtime both PMs 10fixtures/S4 mandatory archive-consumer rerun clean ∷ real set L2_REBUILD=1 L2_MODE=payload-tolerant L2_BUILD_MODE=deep l1-merge.txt⇒l2-20260913T204249Z: both PMs 19binpkgs; stop=#37S4: 8unexpl archive rows all metadata/{DEPEND,RDEPEND} rewrite(pv/libbsd/jq/htop/sudo)=l2-gpkg-dep-metadata-rewrite⇒#39 widened; consume cand rc1 after 5/18 (real selects only 5 portuale archives `[binary N] ::__unknown__`, falls back `[binary N g]`/ebuild, dies extracting oniguruma-6.9.10-6 from image binhost `GnuPG verification failed` no-pubkey)=l2-binpkg-gpg-check⇒NEW backlog #43+owner note; NO env(#37) NO transform(#38) rows ∷ docs: what-this-proves S4/S5 para(live l2-instprep-repro/L2/L1 cmds); scope-backlog §K transforms bullet closed+GPG entry→#43; backlog #38 DONE/#39 widened(dep-rewrite+l2-pkgindex-eapi-missing)/#43 filed; fixture README+docs ebuild doman-compressed claim corrected(pt.1 33B plain<128 SIZE_LIMIT; only BIG.txt.bz2 compressed); docs/030 G0.5+S3 "#38 landed/verification-mode" ∷ step8: cargo fmt --check ok, clippy --release --all-targets 0warn, cargo test --release 1025passed/0failed, pytest tests -q 1567passed/2skipped/8xfailed ∷ ALL remaining KNOWN_FINDINGS+yaml rows hit by S5 runs or S4 repro ⇒ no dead allowlist entry
  POSTSCRIPT 2026-09-13 ∷ backlog #42 FIXED ∷ ebuild_phases::run_clean(real clean phase __dyn_clean) @ real positions: pre-build in merge_one_source_entry+build_one_source_entry+run_buildpkgonly; unconditional post-package in run_buildpkgonly; post-merge in run_merge+merge_one_built_entry iff ¬FEATURES=noclean; run_qmerge clean-free(real doebuild qmerge⇒noclean) ∷ TEST/layers/l2/instprep-repro.sh wipe REMOVED ⇒ TEST/run/l2-instprep-repro.sh rc0 l2-instprep-20260913T222526Z(cells identical; snapshot matches all pt-* targets so setuid race ¬flip) ∷ tests Rust run_clean_*2/run_merge_post_cleans_*/source_merge_pre_cleans_*/buildpkgonly_post_cleans_* + pytest test_emerge_pre_cleans_a_dirty_builddir_before_rebuilding; existing T/build-info asserts now noclean/keepwork ∷ evidence TEST/findings/l2.md "#42"; what-this-proves "emerge cleans the build directory like real"
