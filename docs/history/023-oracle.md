...
| boost | `test_slot_conflict_update.py` (missed-update: mask lower first) | `[boost-build-1.53.0, boost-1.53.0, libcmis-0.3.1]` | same upgrades plus the `libcmis` subslot rebuild (`r`-tagged provider + `rR` consumer, "causing rebuilds") | MATCH | highest-first + solvable growth decides before any mask path; the lower-mask-first ranking never triggers because no conflict arises. Guard, not a ranking probe. The `libcmis` rebuild half closed in #24 S1 (complete-mode gate). |
| virt | `test_slot_conflict_update_virt.py` (bug 692746: mask the all-matched existing node) | `[mysql-connector-c-8.0.17-r3, libmysqlclient-21, DBD-mysql-4.44.0]` | same upgrades plus the `DBD-mysql` subslot rebuild (`r`-tagged provider + `rR` consumer, "causing rebuilds") | MATCH | same as boost: no slot conflict arises, so the 692746 append never triggers. Guard, not a probe. The `DBD-mysql` rebuild half closed in #24 S1 (complete-mode gate). |
...
| bwd | `test_aggressive_backtrack_downgrade.py` (bug 693836, `--update --deep`) | `[]` | `[]` (no `firefox`/`libvpx`/`ffmpeg` lines) | MATCH | no aggressive downgrade on either side. Finer `conflict_downgrade` variants are backlog #35, not #23 (shipped 2026-09-15; its own oracle is `test_or_choices.py::testConflictMissedUpdate`, `TEST/logs/r4-20260915/`). |
| a522084 | `test_solve_non_slot_operator_slot_conflicts.py` (bug 522084, `--update --deep`) | `[app-misc/A-2, app-misc/B-0]` | `[app-misc/A-2, app-misc/B-0]` (both `r`-tagged + "causing rebuilds") | MATCH | version choice was already #23's half; the `B-0` subslot rebuild closed in #24 S1 (complete-mode gate: real `_complete_graph` has no `--deep` condition, `depgraph.py:8562-8648`). |
