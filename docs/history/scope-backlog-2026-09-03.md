# Scope backlog — pre-2026-09-03 snapshot (shipped-item narrative)

> **HISTORIC.** This is [`../scope-backlog.md`](../scope-backlog.md) as it
> stood on 2026-09-03, immediately before a compaction pass that purged
> every "shipped" / "done" annotation from Part 2 and the per-item
> "shipped" recaps from Part 4 (the file had re-accreted a
> shipped-item narrative for every slice — the same drift the 2026-08-31
> rewrite noted). The live file now keeps only genuinely-remaining work,
> the standing non-goals, and a compact distance-to-parity assessment.
>
> Nothing here is lost: every shipped item below is recorded, with its
> cited-source grounding, in [`../what-this-proves.md`](../what-this-proves.md)
> and in `git log`. This snapshot is kept only so the exact wording of
> the scope cuts and narrowings noted alongside each shipped slice
> remains recoverable.
...
- **`package.use` per-level `USE_ORDER` layering.** *(Shipped
  ...
  *(Shipped 2026-09-02: the process-environment **`$USE` at its real
  `env` position** — the highest `USE_ORDER` tier, above the user-level
  `package.use`. New `Config::env_use_tokens` (was folded into the
  weaker `conf` tier); `effective_use_flags` replays it last, so
...
- **Build-phase environment.** *`USE` + compiler/make flags shipped
  2026-09-02:* an `emerge <atom>` source build/merge runs its ebuild
  phases with the resolved `USE` (`MergeOptions::build_env` ←
  `GraphEntry::use_flags_display`) — so `bin/ebuild.sh`'s `use()` works
  and `write_post_install_metadata` USE-reduces the vdb `*DEPEND` against
  the real set — and with `CFLAGS`/`CXXFLAGS`/`CPPFLAGS`/`LDFLAGS`/
  `FFLAGS`/`FCFLAGS`/`ASFLAGS`/`MAKEOPTS`/`CHOST`/`CBUILD`/`CTARGET` from
  the resolved config (`pretend.rs::build_config_env` ←
  `Config::other_vars`). Still `""`/absent: `FEATURES` (the pilot forces
  `FEATURES=""` — see the non-isolation `FEATURES` item below); `USE` /
  build flags for a standalone `ebuild <file> <phase>` (no graph, no
  resolved config) and for `emerge --resume` (`resume_entry` carries no
  resolved USE); the `Packages` *index* `USE` field for an `emerge -b`
  binpkg (not back-filled from build-info).
