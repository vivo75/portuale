...
def test_dispatch_via_symlink_ebuild(multicall_binary, tmp_path):
    ...
    result = subprocess.run(
        [str(ebuild_link), "foo-1.0.ebuild", "qmerge"],
        capture_output=True,
        text=True,
        check=True,
    )
...
def test_explicit_arg_fallback_dispatch(multicall_binary):
    ...
    result = subprocess.run(
        [str(multicall_binary), "ebuild", "foo-1.0.ebuild", "qmerge"],
        capture_output=True,
        text=True,
        check=True,
    )
...
def test_ebuild_accepts_a_real_value_option_without_misreading_its_value(ebuild_binary):
    ...
    result = subprocess.run(
        [str(ebuild_binary), "--color", "y", "foo-1.0.ebuild", "qmerge"],
        capture_output=True,
        text=True,
        check=True,
    )
    ...
    assert 'commands: ["qmerge"]' in result.stdout
...
def test_ebuild_rejects_an_unrecognized_option(ebuild_binary):
    ...
    deviates)."""
    result = subprocess.run(
        [str(ebuild_binary), "--not-a-real-option", "foo-1.0.ebuild", "qmerge"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_ebuild_rejects_a_filename_not_ending_in_dot_ebuild(ebuild_binary):
    result = subprocess.run(
        [str(ebuild_binary), "foo-1.0.tar.gz", "qmerge"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_ebuild_version_is_recognized_but_not_specially_implemented(ebuild_binary):
    ...
    result = subprocess.run(
        [str(ebuild_binary), "--version", "foo-1.0.ebuild", "qmerge"],
        capture_output=True,
        text=True,
        check=False,
    )
