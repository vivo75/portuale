...
| 21 | Real merge/install/filesystem mutation | **shipped** | `2f5a3ddad` (`merge`) + `2a52f7d88` (`unmerge`) + `qmerge`/`config`/`info`/`prerm`/`postrm` + real `CONFIG_PROTECT`/`collision-protect`/`preserve-libs`/`env_update`; **`emerge <atom>` source build + merge shipped 2026-08-31** (`emerge_build::run_source_merge` → `ebuild_merge::run_merge` per resolved entry; `MergeOptions::from_env`) — **`New` + `Upgrade`/`Downgrade`/`Reinstall`** (an in-place same-slot replace unmerges the old version: `ebuild_merge::unmerge_replaced_same_slot`, factored out of `merge_binpkg`, also wired into `merge_after_install` so `ebuild <file> merge` of v2 over v1 stops orphaning v1's files); v1 cuts: no `--keep-going`, no mixed source+binary, no preserve-libs/reverse-dep check on the replace |
...
6. **Remote binpkg fetching** — the `--pretend` half of `--getbinpkg`/
   ...
   **Still open:** live `layout.conf` negotiation, `Packages.gz`, resume
   support, preserve-libs / reverse-dep check on a binpkg replace,
