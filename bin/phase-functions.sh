...
#
# This exists to avoid `__save_ebuild_env | __filter_readonly_variables`: both
# stages are shell functions, and their combined output routinely exceeds the
# OS pipe buffer (~64KiB on Linux) once a few eclasses are in scope. A shell
# that runs a *non-last* function pipeline stage inline -- to completion --
# rather than as a concurrently-running process then blocks forever on the
# stage's own `write()` to a pipe nothing is draining yet. bash always forks
# every pipeline stage, so it never hits this; brush (the alternate backend
# `bin/ebuild.sh` can run under) historically did not. Staging through a
